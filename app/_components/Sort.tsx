...
function SortLists({ sortProps, initialState }: any) {
  ...
  return (
    <button
      className="text-xs font-medium border border-gray-400 rounded-sm py-4 px-2 focus:ring-0 outline-none relative min-w-28 bg-white transition-all duration-500 flex justify-between items-center"
...
function List({ state, onClick, closeModal, sortProps }: ListProps) {
  ...
  return (
    <ul className="absolute text-xs font-medium border border-gray-400 rounded-sm focus:ring-0 outline-none min-w-28 left-0 top-14 bg-white shadow-2xl transition-all duration-500">
